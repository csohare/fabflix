...
public class SingleMovieServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            String top20Query = "SELECT * FROM (SELECT * FROM movies WHERE movies.id = 'tt0498362') as movieInfo, (SELECT  group_concat(starId), group_concat(starName), genreId, genreName FROM (SELECT starId, name as starName FROM stars_in_movies as sim, stars WHERE sim.movieId = 'tt0498362' and stars.id = sim.starId) as movieStars, (SELECT group_concat(genreId) as genreId, group_concat(name) as genreName FROM genres_in_movies as gim, genres WHERE gim.movieId = 'tt0498362' and genres.id = gim.genreId) as genreStars GROUP BY genreId, genreName) as mstarGenreSELECT * FROM (SELECT * FROM movies WHERE movies.id = 'tt0498362') as movieInfo, (SELECT  group_concat(starId), group_concat(starName), genreId, genreName FROM (SELECT starId, name as starName FROM stars_in_movies as sim, stars WHERE sim.movieId = 'tt0498362' and stars.id = sim.starId) as movieStars, (SELECT group_concat(genreId) as genreId, group_concat(name) as genreName FROM genres_in_movies as gim, genres WHERE gim.movieId = 'tt0498362' and genres.id = gim.genreId) as genreStars GROUP BY genreId, genreName) as mstarGenre"
